...
<div class="w-full max-w-3xl mx-auto px-4 lg:px-0">
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-600 mt-1">
            Kurangi stok produk dari gudang
        </p>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <form action="{{ route('dashboard.stock-out.store') }}"
              method="POST"
              id="stockOutForm"
              class="p-6 space-y-6">
            @csrf
                ...
                <div class="relative">
                    <div class="relative">
                        ...
                        <input type="text"
                               ...
                               autocomplete="off"
                               class="w-full pl-10 pr-10 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-600 focus:border-transparent transition-all">
                    ...
                    <div id="selectedProduct" class="hidden mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                ...
                                <p class="text-sm text-gray-600 mt-1">
                                    Stok tersedia: <span id="selectedProductStock" class="font-semibold text-green-600"></span>
            ...
            <div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <input type="number"
                           ...
                           min="1"
                           value="{{ old('qty', 1) }}"
                           class="flex-1 text-center border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-red-600 focus:border-transparent transition-all"
                           disabled>
                    <button type="button"
                            id="incrementQty"
                            class="w-10 h-10 flex items-center justify-center border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            <div>
                ...
                <textarea name="description"
                          rows="3"
                          placeholder="Contoh: Terjual, Rusak, Retur, dll."
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-red-600 focus:border-transparent transition-all resize-none">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div class="flex flex-col sm:flex-row gap-3 justify-end">
                ...
                <button type="submit"
                        id="submitBtn"
                        disabled
                        class="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-red-600 text-white text-sm font-medium rounded-lg hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"/>
                    </svg>
                    Kurangi Stok
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    productSearch.addEventListener('input', function() {
        ...
        searchTimeout = setTimeout(() => {
            fetch(`{{ route('dashboard.stock-out.search-products') }}?search=${encodeURIComponent(query)}`)
    ...
    function validateQty() {
        ...
        if (qty > currentStock) {
            qtyError.textContent = `Jumlah melebihi stok yang tersedia (${currentStock})`;
            qtyError.classList.remove('hidden');
            return false;
        }
